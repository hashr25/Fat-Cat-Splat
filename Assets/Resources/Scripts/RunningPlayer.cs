...
public class RunningPlayer : MonoBehaviour {
	...
	private void LateUpdate()
    {
        if (!colliderChanged)
        {
			PolygonCollider2D original = GetComponent<PolygonCollider2D>();
			Destroy(original);
    ...
    void MovementAndCollisionDetection () {
		...
		if ((Input.GetKeyDown (KeyCode.Space) || (Input.touchCount > 0 && Input.touches[0].phase == TouchPhase.Began)) && SceneManager.GetActiveScene().name != "CharacterStore")  {
			/*if (wallSliding) {
	...
	public void UpdateSprite(RuntimeAnimatorController animator)
    {
		if(anim == null)
        {
			anim = GetComponent<Animator>();
